...
document.addEventListener("pointerover", updateLummmenHoverStart);

let latestLummmenMove = null;
let lastLummmenMove = null;
let lastLummmenScrollY = window.scrollY;
...
function pushLummmenScrollData() {
  ...
  lastLummmenScrollPercent = percent;
}

function scheduleLummmenScrollData() {
  if (lummmenScrollFrame) return;

  const requestFrame = window.requestAnimationFrame || function (callback) {
    return setTimeout(callback, 16);
  };
  lummmenScrollFrame = requestFrame(pushLummmenScrollData);
}
